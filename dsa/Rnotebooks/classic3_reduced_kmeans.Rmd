...
```{r}
df.classic3 <- read.csv("~/Documents/Uparis/M1MLSD2223/ue_data2/text-clustering/core/datasets/data/classic3.csv")
```
...
df.classic3 = dplyr::tibble(df.classic3[-1])
head(df.classic3)
```

```{r}
# convert as factor
df.classic3 = df.classic3 |>
  dplyr::mutate(label = factor(label))
```

```{r}
df.classic3.docs = dplyr::tibble(df.classic3["text"])
df.classic3.labels = dplyr::tibble(df.classic3["label"])
```

```{r}
str(df.classic3.labels)
```
...
df.classic3.docs.as.vec = dplyr::pull(df.classic3.docs, text)
df.classic3.labels.as.vec = dplyr::pull(df.classic3.labels, label)
```

# Tokenizer
```{r}
clust_vec = textTinyR::tokenize_transform_vec_docs(
  object = df.classic3.docs.as.vec, 
  as_token = T,
  to_lower = T, 
...
utl = textTinyR::sparse_term_matrix$new(
  vector_data = df.classic3.docs.as.vec, 
  file_data = NULL,
...
table(df.classic3.labels.as.vec, out.cluspca$cluster)
```
